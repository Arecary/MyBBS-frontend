    ...
    <el-card style="width: 50%">
      <el-form ref="formRef" :model="user" :rules="rules" label-width="100px" style="padding-right: 50px">
        ...
        <el-form-item label="Confirm" prop="confirmPassword">
          <el-input show-password v-model="user.confirmPassword" placeholder="confirm new passwprd"></el-input>
        </el-form-item>
        <div style="text-align: center; margin-bottom: 20px">
          <el-button type="primary" @click="update">confirm edit</el-button>
        </div>
      </el-form>
    </el-card>
  </div>
  ...
  data() {
    const validatePassword = (rule, value, callback) => {
      if (value === '') {
        ...
        callback(new Error('confirm password not correct'))
      } else {
        callback()
      }
    }
